//
//  EventViewController.swift
//  BGClan
//
//  Created by Shoaib Akhtar on 16/05/24.
//

import UIKit

class EventViewController: UIViewController {

    required init?(coder: NSCoder) {
        super.init(coder: coder)
        self.tabBarItem.title = "Events"
        self .tabBarItem.image = UIImage(systemName: "e.square.fill")
    }
    
    @IBOutlet weak var segmentControllerEvents: UISegmentedControl!
    @IBOutlet weak var textLabelEvent1: UILabel!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        textLabelEvent1.text = "There will be upcoming events"

        // Do any additional setup after loading the view.
    }
    
    @IBAction func segmentControllerEventsIBAction(_ sender: UISegmentedControl) {
        switch segmentControllerEvents.selectedSegmentIndex{
        case 0:
            textLabelEvent1.text = "There will be upcoming events"
        case 1:
            textLabelEvent1.text = "There will be past events"
        default:
            break
        }
    }
    

    /*
    // MARK: - Navigation

    // In a storyboard-based application, you will often want to do a little preparation before navigation
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        // Get the new view controller using segue.destination.
        // Pass the selected object to the new view controller.
    }
    */

}
